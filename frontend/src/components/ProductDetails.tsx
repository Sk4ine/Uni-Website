...
function ProductImages({ imagePaths }: { imagePaths: string[] }) {
  ...
  for (let i = 1; i < imagePaths.length; i++) {
    altImages.push(
      <ImageButton key={i} imagePath={imagePaths[i]}></ImageButton>,
    );
  }
  ...
  return (
    <div className="flex flex-col items-center w-[45%]">
      <img
        src={imagePaths[0]}
        className="w-full size-contain aspect-square object-contain rounded-3xl"
      ></img>
...
function ImageButton({ imagePath }: { imagePath: string }) {
  return (
    <button
      ...
      className={`size-16 bg-contain bg-center cursor-pointer rounded-2xl`}
    ></button>
  );
}
...
function ProductDescription({ product }: { product: Product }) {
  ...
  return (
    <div className="flex flex-col items-center w-[55%]">
      <div className="flex flex-col items-center font-default text-[#555555] text-[2.8rem]">
        <p>{product.name}</p>
      ...
      {cartContext.cartProductList.find((p) => p.productID == product.id) ? (
        <Link
          to="/cart"
          className="w-fit px-16 py-2.5 mt-8 font-default text-[#D5778D] text-3xl bg-[#F5D4D5] hover:bg-[#E6C8C9] rounded-3xl cursor-pointer"
        >
          Перейти в корзину
        </Link>
      ) : (
  ...
  return (
    <div className="flex flex-col w-full px-9 mt-6 font-default text-[#B4A1A6] text-2xl">
      ...
      <div className="flex flex-col gap-1 mt-3">
        ...
        <DescriptionItem
          itemName="Вес"
          itemValue={product.weightGrams + " г"}
        ></DescriptionItem>
        <DescriptionItem
          itemName="Производство"
          itemValue={product.countryOfOrigin}
        ></DescriptionItem>
      </div>
    </div>
  );
...
export function ReviewCard({ review }: { review: Review }) {
  const imageVisibility: string =
    review.attachedImagePath !== undefined ? "" : "hidden";
  ...
  return (
    <div className="flex justify-between w-2xl rounded-2xl shadow-[-2px_0px_4px_rgb(0,0,0,0.25),2px_4px_4px_rgb(0,0,0,0.25)] p-4">
      <div className="w-[10%] pl-2">
        <img
          src={review.user.logoImagePath}
          className="size-12 rounded-full"
        ></img>
      </div>
      <div className="flex flex-col w-[65%] pl-4 font-default text-[#555555] text-3xl">
        <p>
          {review.user.firstName}, {review.date.getDate()}{" "}
          {reviewDateMonths[review.date.getMonth()]} {review.date.getFullYear()}
        </p>
      ...
      <div className="w-[25%]">
        <img
          src={review.attachedImagePath}
          className={`size-40 aspect rounded-2xl ${imageVisibility}`}
        ></img>
      </div>
    </div>
  );
...
function RatingStars({ rating }: { rating: number }) {
  ...
  return (
    <div className="w-[125px] h-[23px] mt-1">
      <div
        style={{ maskImage: `url(${ratingStarImage})` }}
        className="bg-[#B5ABA1] w-full h-full"
      >
        <div
          style={{ width: `${fillPercent}%` }}
          className={`bg-[#FFD900] h-full mask-repeat-x`}
        ></div>
      </div>
    </div>
  );
...
function Rating({ product }: { product: Product }) {
  ...
  for (let i = 1; i <= 5; i++) {
    ratingPercents.push(
      (product.customerReviews.filter((review) => review.rating == i).length /
        product.customerReviews.length) *
        100,
    );
  }
  ...
  return (
    <div className="flex flex-col">
      <div className="flex justify-end items-center">
        <p className="font-default text-[#555555] text-6xl">
          {product.rating.toFixed(1)}
        </p>
        <img
          src={ratingStarLargeImage}
          className="size-11 ml-2 pb-1 select-none pointer-events-none"
        ></img>
      </div>
  ...
  return (
    <div className="flex items-center h-5">
      ...
      <div className="w-80 h-2 rounded-2xl bg-[#D9D9D9]">
        <div
          style={{ width: `${percent}%` }}
          className={`h-full rounded-2xl bg-[#D5778D]`}
        ></div>
      </div>
    </div>
  );
