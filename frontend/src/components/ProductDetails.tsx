...
function ProductInfo({ product }: { product: Product }) {
  return (
    <div className="flex justify-between w-[1100px] mt-10">
      <ProductImages name={product.name} imagePaths={product.imagePaths}></ProductImages>
      <ProductDescription product={product}></ProductDescription>
    </div>
  );
}

function ProductImages({ name, imagePaths }: { name: string; imagePaths: string[] }) {
  const altImages: React.ReactNode[] = [];
  ...
  return (
    <div className="flex flex-col items-center w-[45%]">
      ...
      ></img>
      <div className="flex justify-center items-center mt-4 gap-3">{altImages}</div>
    </div>
  );
}
  ...
  return (
    <div className="flex justify-between w-2xl rounded-2xl shadow-[-2px_0px_4px_rgb(0,0,0,0.25),2px_4px_4px_rgb(0,0,0,0.25)] p-4">
      <div className="w-[10%] pl-2">
        <img src={review.user.logoImagePath} alt="userLogo" className="size-12 rounded-full"></img>
      </div>
