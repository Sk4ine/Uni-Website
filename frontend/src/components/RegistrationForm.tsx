    ...
    >
      ...
      <div className="w-full flex flex-col items-center gap-2 mt-4 flex-wrap">
        {registrationFailMessage.length != 0 ? (
          <p className="font-default text-[#BC4241] text-xl text-center">
            {registrationFailMessage}
          </p>
        ) : null}
        <button
          type="submit"
          className="w-full font-default text-[#D5778D] text-2xl py-2 rounded-2xl bg-[#F5D4D5] hover:bg-[#E6C8C9] cursor-pointer"
        >
          Зарегистрироваться
        </button>
      </div>
      <div className="flex flex-col items-center font-default text-[#555555] text-xl">
        ...
        <Link to={"/auth/login"} className="text-[#D5778D]">
          {" "}
          Войти
        </Link>
      </div>
