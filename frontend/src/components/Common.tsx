    ...
    <div className="w-80 h-[27rem] flex flex-col items-center justify-between bg-white shadow-[-2px_0px_4px_rgb(0,0,0,0.25),2px_4px_4px_rgb(0,0,0,0.25)] rounded-2xl animate-fade-in">
      ...
      {cartContext.cartProductList.find((p) => p.productID == product.id) ? (
        ...
        >
          ...
          <FontAwesomeIcon
            className="object-none text-[#D5778D] ml-2"
            icon={faCircleCheck}
            size="2xl"
          ></FontAwesomeIcon>
        </Link>
  ...
  return (
    <div className="flex flex-wrap justify-start pl-[11px] w-[1344px] mt-9 gap-x-3.5 gap-y-6">
      {isLoading || productCards.length === 0
        ? [...Array(4)].map((_, index) => <LoadingProductCard key={index} />)
        : productCards}
    </div>
  );
}
...
export function InputField({
  ...
  return (
    <div className="flex flex-col items-start ">
      <label htmlFor={fieldID} className="font-default text-[#555555] text-2xl">
        {fieldName}
      </label>
      <input
        ...
        required={required}
        disabled={disabled}
        className="w-80 text-[#555555] text-xl px-4 py-1.5 rounded-2xl outline-none disabled:bg-[#EBEBEB] shadow-[-2px_0px_4px_rgb(0,0,0,0.25),2px_4px_4px_rgb(0,0,0,0.25)]"
      ></input>
    </div>
  );
}
