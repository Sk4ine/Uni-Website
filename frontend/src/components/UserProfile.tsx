...
function LoadingUserInfo() {
  return <div className="flex justify-between w-[1100px] mt-12"></div>;
}
...
function OrdersList() {
  ...
  return (
    <div className="flex flex-col items-center mt-8">
      ...
      {userOrders.length == 0 ? (
        <p className="font-default text-[#B4A1A6] text-4xl mt-8">
          У вас нет заказов
        </p>
      ) : (
...
function OrderCard({ order }: { order: Order }) {
  ...
  const product: Product = productListContext.filter(
