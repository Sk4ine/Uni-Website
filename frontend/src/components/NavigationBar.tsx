...
function NavigationBarButton({ text, url }: { text: string; url: string }) {
  ...
  return (
    <Link
      to={url}
      className={`${activeColor} font-default h-[65%] m-2 mb-0 p-3 text-center content-center text-3xl`}
    >
      {text}
    </Link>
  );
...
function UserLogoButton() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        <div
          style={{ maskImage: `url(${defaultUserLogo})` }}
          className={`${activeColor} size-[50px] mask-contain mask-no-repeat mask-center`}
        ></div>
      </button>
      ...
      {isOpen && (
        <div className="absolute translate-x-[-50%] left-[50%] mt-2 bg-white rounded-xl shadow-lg ring-1 ring-[#555555]/10 z-10">
          <div className="flex flex-col text-lg font-default">
            <Link
              to="/user-profile"
              className="pl-4 pr-10 py-2 text-[#555555] hover:bg-gray-100 rounded-t-xl"
            >
              Профиль
            </Link>
            ...
            {authContext.isAdmin && (
              <Link
                to="/admin-panel"
                className="text-nowrap pl-4 pr-10 py-2 text-[#555555] hover:bg-gray-100"
              >
                Админ панель
              </Link>
            )}
            ...
            <button
              ...
              className="text-left pl-4 pr-10 py-2 text-[#BC4241] hover:bg-gray-100 rounded-b-xl cursor-pointer"
            >
              Выйти
            </button>
          </div>
    ...
    <div className="ml-2 h-[65%] content-center">
      <form className="flex justify-between w-72 border-2 border-[#878078] rounded-4xl p-2">
        <input
          type="text"
          placeholder="Найти"
          className="inline font-default text-xl text-[#878078] ml-3 w-[80%] placeholder:text-[#878078] outline-none indent-1"
        ></input>
        <FontAwesomeIcon
          className="w-[20%] mt-1 text-[#878078]"
    ...
    >
      <div className="relative size-[50px]">
        <div
          ...
          className={`${activeColor} size-full mask-contain mask-no-repeat mask-center`}
        ></div>
        <div
          style={{
            visibility:
        ...
        >
          <p className="font-default text-[16px] text-white pt-[0.115rem]">
            {cartContext.cartProductList.length}
          </p>
        </div>
      </div>
    </Link>
