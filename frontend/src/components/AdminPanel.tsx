...
import { addCategory, addProduct, deleteCategory, deleteProduct, updateCategory, updateProduct } from "../api/requests/admin";
  ...
  switch (activeTable) {
    case "categories":
      recordNodes = (tableRecords as ProductCategory[]).map(category => (
        <tr key={category.id} className="border-2 border-font-darkgray/50 border-x-0 last:border-b-0">
          <td className="font-default text-font-darkgray text-xl py-1 text-center border-r-2 border-font-darkgray/50">{category.id}</td>
    ...
    case "products":
      recordNodes = (tableRecords as Product[]).map(product => {
        const categoryName: string | undefined = categoryList.find(category => category.id == product.categoryID)?.name;
        ...
        return (
          <tr key={product.id} className="border-2 border-font-darkgray/50 border-x-0 last:border-b-0">
            <td className="font-default text-font-darkgray text-xl py-1 text-center border-r-2 border-font-darkgray/50">{product.id}</td>
            <ProductCell id={product.id} name={product.name} price={product.price} categoryName={categoryName} imagePath={product.imagePaths[0]}></ProductCell>
          </tr>
        );
    });
      break;
...
function ProductCell({id, name, price, categoryName} : {id: number, name: string, price: number, categoryName: string, imagePath: string}) {
  return (
    <CellBase>
...
function EditButtons({recordID} : {recordID: number}) {
  ...
  return (
    <div className="flex justify-between items-center font-default gap-2 mr-4">
      <button onClick={handleEdit} className="bg-[#E2E2E2] hover:bg-[#D1D1D1] py-1 px-3 rounded-xl cursor-pointer">Редактировать</button>
      <button onClick={handleDelete} className="bg-[#F5D4D5] hover:bg-[#E6C8C9] py-1 px-3 rounded-xl cursor-pointer">Удалить</button>
    </div>
  )
}

function CategoryEditForm() {
  const {editIsOpen, tableRecords, setEditIsOpen, selectedRecord, setSelectedRecord, updateTableRecords} = useAdminPanelContext();
  ...
  const categoryData: ProductCategory | undefined = tableRecords.find(category => category.id == selectedRecord) as ProductCategory | undefined;
    ...
    <div className="flex justify-center items-center fixed inset-0 bg-black/50 z-50">
      <form action={handleSave} className="flex flex-col items-center font-default text-xl bg-white rounded-2xl px-4 pt-4 pb-2">
        ...
        <div className="flex flex-col items-center py-4 px-6">
          ...
          <EditFormInputField defaultValue={categoryData.name} type="text" name="name" labelText="Название"></EditFormInputField>
...
function ProductEditForm() {
  ...
  const productData: Product | undefined = tableRecords.find(product => product.id == selectedRecord) as Product | undefined;
  ...
  const categoryOptions = categoryList.map((category, index) => (
    <option key={index} value={category.id} className="font-default text-xl">{category.name}</option>
  ));
        ...
        { loadingCategories ? (
          ...
          <>
            ...
            <div className="flex flex-col items-center gap-4 py-4 px-6 max-h-[75vh] overflow-y-scroll">
              ...
              <EditFormInputField defaultValue={productData.materials.join(",")} type="text" name="materials" labelText="Материалы"></EditFormInputField>
...
function DeleteConfirmDialog() {
  const {activeTable, updateTableRecords, selectedRecord, setSelectedRecord, setDeleteIsOpen, tableRecords, deleteIsOpen} = useAdminPanelContext();
  ...
  const recordData: ProductCategory | Product | undefined = tableRecords.find(category => category.id == selectedRecord);
    ...
    <div className="flex justify-center items-center fixed inset-0 bg-black/50 z-50">
      <div className="flex flex-col items-center bg-white rounded-2xl px-4 py-4">
        ...
        <div className="flex justify-between items-center font-default text-2xl gap-5 mt-4">
          <button onClick={handleConfirm} className="bg-[#F5D4D5] hover:bg-[#E6C8C9] py-1 px-3 rounded-xl cursor-pointer">Подтвердить</button>
          <button onClick={handleCancel} className="bg-[#E2E2E2] hover:bg-[#D1D1D1] py-1 px-3 rounded-xl cursor-pointer">Отменить</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <tr>
      <td colSpan={2} className="text-center">
        <button onClick={handleAdd} className="w-full h-full cursor-pointer py-2 text-font-pink hover:text-font-pink-hover hover:bg-gray-50 rounded-b-xl">
