...
export async function getProductList(): Promise<Product[]> {
  try {
    ...
    for(let i = 0; i < productList.length; i++) {
      ...
      try {
        imageResponse = await axios.get<Blob>(`${API_BASE_URL}/static/productImages/${productList[i].id}`, { responseType: 'blob' });
      } catch (error) {
...
export async function getProductByID(id: number): Promise<Product> {
  try {
    ...
    try {
      imageResponse = await axios.get<Blob>(`${API_BASE_URL}/static/productImages/${product.id}`, { responseType: 'blob' });
    } catch (error) {
      if(isAxiosError(error)) {
        if (error.status == 404) {
